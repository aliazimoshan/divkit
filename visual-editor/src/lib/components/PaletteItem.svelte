<script lang="ts">
  ...
  import {
    PALETTE_ERRORS_CTX,
    type PaletteErrorsContext,
  } from '../ctx/paletteErrorsContext';
  ...
  function rebuildVal(): void {
    ...
    if (parsed) {
      ...
      isSelfUpdate = true;
      state.pushCommand(
        new ChangePaletteItemCommand(
          value.id,
          $previewThemeStore,
          stringifyColorToDivKit(parsed),
        ),
      );
      dispatch('change');
    }
  ...
  function onPreviewClick(): void {
    color2Dialog().show({
      ...
      callback(newValue) {
        state.pushCommand(
          new ChangePaletteItemCommand(value.id, $previewThemeStore, newValue),
        );
      },
    });
  }
  ...
  function onNameInput(): void {
    if (nameVal.length > MAX_NAME_LENGTH) {
      ...
      const cursorStart = calcSelectionOffset(
        selection,
        nameElem,
        'start',
        false,
      ...
      tick().then(() => {
        ...
        setSelectionOffset(
          ...
          range,
          'start',
          Math.min(cursorStart, MAX_NAME_LENGTH),
          false,
        );
        setSelectionOffset(
          selection,
          ...
          range,
          'end',
          Math.min(cursorEnd, MAX_NAME_LENGTH),
          false,
        );
...
>
  ...
  <button class="palette-item__preview" on:click={onPreviewClick}>
    <ColorPreview
      color={value[$previewThemeStore]}
      mix="palette-item__preview-inner"
    />
  </button>
  ...
  <div class="palette-item__symbol">%</div>
</div>
