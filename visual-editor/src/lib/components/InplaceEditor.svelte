<script lang="ts">
  ...
  const lineHeight =
    json.line_height !== undefined ? json.line_height / fontSize : 1.25;
    ...
    list.forEach((item) => {
      ...
      if (index > prevIndex) {
        ...
        newRenderList.push({
          ...
          textStyles,
          actions:
            (item.type === 'rangeEnd' && item.range?.actions) || undefined,
        });
      }
      ...
      if (item.type === 'rangeStart' && range) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        let textStyles2 = Object.assign(
          { ...rootTextStyles },
          ...(activeRanges as any[]),
        ) as TextStyles;
        let imageWidth = pxToEm(
          (((item.image.width && item.image.width.value) || 20) * 10) /
            (textStyles2.font_size || 12),
        );
        let imageHeight = pxToEm(
          (((item.image.height && item.image.height.value) || 20) * 10) /
            (textStyles2.font_size || 12),
        );
        const wrapperStyle: Style = {
          'font-size': pxToEm(
            ((Number(textStyles2.font_size) || 12) * 10) / fontSize,
          ),
        };
  ...
  function draw(renderList: RenderItem[]): void {
    ...
    elem.innerHTML = renderList.reduce((acc, item) => {
      ...
      if ('text' in item) {
        ...
        html =
          `<span data-image="${htmlFilter(JSON.stringify(images[item.index]))}" data-image-index="${htmlFilter(String(item.index))}" class="inplace-editor__image-wrapper" style="${htmlFilter(makeStyle(item.image.wrapperStyle) || '')}">` +
          `<img class="inplace-editor__image" src=${htmlFilter(item.image.url)} loading="lazy" decoding="async" ` +
          `aria-hidden="true" alt="" style=${htmlFilter(
            makeStyle({
              width: item.image.width,
              height: item.image.height,
              // Normalizes line-height for the containing text line
              'margin-top': customLineHeight
                ? `-${item.image.height}`
                : undefined,
              'margin-bottom': customLineHeight
                ? `-${item.image.height}`
                : undefined,
              filter: item.image.svgFilterId
                ? `url(#${item.image.svgFilterId})`
                : undefined,
            }) || '',
          )}` +
          '></span>';
    ...
    Array.from(elem.querySelectorAll('.inplace-editor__link')).forEach(
      (link) => {
  ...
  function restoreSelection(
    start = selectionStartTextOnly,
    end = selectionEndTextOnly,
    includeImages = false,
  ): void {
    const sel = getSelection();
    if (sel) {
  ...
  function onSelectionChange(): void {
    if (ownerDocument.activeElement === elem) {
      const selection = getSelection();
      selectionStartTextOnly = calcSelectionOffset(
        selection,
        elem,
        'start',
        false,
  ...
  function updateState(): void {
    ...
    weight = '';
    underline = Boolean(
      findVal('underline', 'single') || rootTextStyles.underline === 'single',
    );
    strike = Boolean(
      findVal('strike', 'single') || rootTextStyles.strike === 'single',
    );
  }
  ...
  function onUnderline(): void {
    ...
    setRange(
      'underline',
      underline ? undefined : 'single',
      selectionStartTextOnly,
      selectionEndTextOnly,
    );
    restoreSelection();
  }
...
>
  ...
  >
    <div class="inplace-editor__toolbar-inner" bind:this={toolbarElem}>
      ...
      >
        <div
          class="inplace-editor__select-icon inplace-editor__select-icon_align_{textAlign}"
        ></div>
      </button>
      <div class="inplace-editor__button2 inplace-editor__button2_select">
        <div
          class="inplace-editor__select-icon inplace-editor__select-icon_weight"
        ></div>
        <select
      ...
      >
        <div
          class="inplace-editor__select-icon inplace-editor__select-icon_underline"
        ></div>
      </button>
      <button
        class="inplace-editor__button2"
      ...
      >
        <div
          class="inplace-editor__select-icon inplace-editor__select-icon_strike"
        ></div>
      </button>
      ...
      >
        <div
          class="inplace-editor__select-icon inplace-editor__select-icon_actions"
        ></div>
      </button>
      <button
        class="inplace-editor__button2"
      ...
      >
        <div
          class="inplace-editor__select-icon inplace-editor__select-icon_image"
        ></div>
      </button>
    </div>
  </div>
...
<style>
  ...
  .inplace-editor_disabled
    .inplace-editor__button2:not(.inplace-editor__button2_color) {
    cursor: default;
    opacity: 0.3;
  }

  .inplace-editor:not(.inplace-editor_disabled)
    .inplace-editor__button2:not(.inplace-editor__button2_disabled):hover {
  ...
  .inplace-editor:not(.inplace-editor_disabled)
    .inplace-editor__button2_text:hover,
  ...
  :global(
    .inplace-editor__color-preview.inplace-editor__color-preview.inplace-editor__color-preview
  ) {
    width: 20px;
    height: 20px;
    border-radius: 4px;
