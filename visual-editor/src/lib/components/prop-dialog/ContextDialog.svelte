<script lang="ts">
  ...
  function calcStyles(): [number, number] {
    ...
    if (dir === 'auto') {
      dir =
        targetBbox.left + targetBbox.width / 2 < window.innerWidth / 2
          ? 'right'
          : 'left';
    }
  ...
  function onOutsideClick(event: MouseEvent): void {
    if (isFullyShowed) {
      ...
      if (
        ...
        !path.some((it) => it instanceof HTMLDialogElement)
      ) {
        isHiding = true;
      }
    }
...
<svelte:window on:click={onOutsideClick} on:resize={update} />
...
>
  ...
  >
    ...
    {#if hasClose}
      <button class="context-dialog__close" on:click={onCloseClick}></button>
    {/if}
  </div>
</dialog>
...
<style>
  .context-dialog {
    ...
    animation: context-dialog__show_left 0.2s ease-in-out;
  }
