<script lang="ts">
  ...
  function onInput(): void {
    ...
    suggestLoaded = false;
    getTranslationSuggest(requestValue, $locale)
      .then((list) => {
  ...
  afterUpdate(() => {
    if (suggestElem && !suggestElem.open) {
      ...
      suggestDirectionUp =
        freeSpaceBottom < DEFAULT_SUGGEST_MAX_HEIGHT &&
        freeSpaceBottom < contentBbox.top;
      suggestMaxHeight = Math.min(
        DEFAULT_SUGGEST_MAX_HEIGHT,
        suggestDirectionUp ? contentBbox.top : freeSpaceBottom,
      );

      suggestElem.show();
      input.focus();
    } else if (suggestElem && suggestActive !== -1) {
      const active = suggestElem.querySelector<HTMLElement>(
...
{#if isShown && target}
  <ContextDialog
    ...
    on:close={onClose}
  >
    <div bind:this={contentElem} class="tanker2-dialog__content">
      ...
      <div class="tanker2-dialog__input-box">
        <input
          ...
          class:tanker2-dialog__input_error={suggestInitiallyLoaded &&
            suggestLoaded &&
            !suggestList.length}
          autocomplete="off"
          autocorrect="off"
