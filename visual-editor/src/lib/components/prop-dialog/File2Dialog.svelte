<script lang="ts">
  ...
  import {
    APP_CTX,
    type AppContext,
    type File2DialogCallback,
    type File2DialogShowProps,
    type File2DialogValue,
  } from '../../ctx/appContext';
  ...
  const { uploadFile, previewWarnFileLimit, previewErrorFileLimit } =
  ...
  const FILE_FILTER_SETS = Object.keys(FILE_FILTER).reduce<
    Record<string, Set<string>>
  >((acc, item) => {
    acc[item] = new Set(
      FILE_FILTER[item as keyof typeof FILE_FILTER]
        .split(',')
        .map((it) => it.trim()),
    );
    return acc;
  ...
  $: placeholder = subtype
    ? $l10nString(`file.${commonSubtype}_placeholder`)
    : '';

  $: fileFilter = subtype
    ? FILE_FILTER[commonSubtype as keyof typeof FILE_FILTER]
    : '';
  ...
  function getFile(list: FileList): File | undefined {
    return Array.from(list).find((it) => {
      return (
        it.type &&
        FILE_FILTER_SETS[commonSubtype] &&
        FILE_FILTER_SETS[commonSubtype].has(it.type)
      );
    });
  }
  ...
  >
    ...
    >
      <div class="file2-dialog__standard-content">
        ...
        >
          <div
            class="file2-dialog__file-preview-container"
          ...
          >
            {#if value.url}
              ...
              {:else if commonSubtype === 'video'}
                ...
                <video class="file2-dialog__video-preview" src={value.url}
                ></video>
              {/if}
            {/if}
          </div>
        ...
        {#if showError}
          ...
          >
            {$l10nString(
              showError === 'big' || showError === 'big-warn'
                ? 'file.too_big'
                : `file.${commonSubtype}_error`,
            )}
          </div>
        {/if}
...
<style>
  ...
  .file2-dialog__drop-area:not(.file2-dialog__drop-area_disabled)
    .file2-dialog__file-select_always-shown {
    opacity: 1;
  }
