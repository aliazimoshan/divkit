...
<script lang="ts">
  ...
  $: if (subtype.startsWith('typed:')) {
    ...
    if (typedType in TYPED_TO_SCHEMA_MAP) {
      typedControls = parseControls(
        TYPED_TO_SCHEMA_MAP[typedType as keyof typeof TYPED_TO_SCHEMA_MAP],
      );
    } else {
      typedControls = [];
    }
...
{#if isShown && target}
  <ContextDialog {target} canMove={true} overflow="visible" on:close={onClose}>
