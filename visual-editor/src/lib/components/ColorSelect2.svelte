...
<script lang="ts">
  ...
  function updateSpectrumColor(event: PointerEvent): void {
    ...
    const x = Math.max(
      0,
      Math.min(1, (event.clientX - spectrumBbox.left) / spectrumBbox.width),
    );
    const y = Math.max(
      0,
      Math.min(1, (event.clientY - spectrumBbox.top) / spectrumBbox.height),
    );
...
<div class="color-select" class:color-select_disabled={disabled}>
  ...
  <div class="color-select__values">
    ...
    <div
      class="color-select__range color-select__alpha"
      style:--value={alpha}
      style:--color={cssColorWithoutAlpha}
    >
    ...
    <div class="color-select__input">
      <ColorInput readOnly={disabled} bind:value on:change={onInput} />
    </div>
  </div>
</div>
