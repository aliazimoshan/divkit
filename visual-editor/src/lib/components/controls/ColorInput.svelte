<script lang="ts">
  ...
  function onPreviewClick(): void {
    ...
    color2Dialog().show({
      ...
      disabled: readOnly,
      direction:
        elem.getBoundingClientRect().right < innerWidth / 2 ? 'right' : 'left',
...
<div class="color-input" class:color-input_readonly={readOnly} bind:this={elem}>
  {#if hasDialog}
    <button
  ...
  {:else}
    ...
    <div class="color-input__symbol">%</div>
  {/if}
</div>
...
<style>
  ...
  :global(
    .color-input__preview-inner.color-input__preview-inner.color-input__preview-inner
  ) {
    position: absolute;
    top: 0;
    left: 0;
