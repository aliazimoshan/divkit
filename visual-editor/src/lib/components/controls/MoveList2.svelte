<script lang="ts">
  ...
  function moveStart(
    ...
    const startOffsetY =
      event.detail.event.clientY - elem.getBoundingClientRect().top;
    ...
    const boxes = Array.from(listElem.children).map((it) =>
    ...
    function pointermove(event: PointerEvent): void {
      ...
      for (let i = 1; i <= filteredBoxes.length; ++i) {
        if (
          event.clientY - startOffsetY >
          newTop - filteredBoxes[i - 1].height / 2
        ) {
          bestIndex = i;
        }
      ...
      offset = Math.min(
        offset,
        boxes[boxes.length - 1].bottom - decidedTop - currentHeight,
      );
...
<ul class="move-list__list" bind:this={listElem}>
  {#each list || [] as item (item.__key)}
    ...
    <li
      ...
      animate:flip={{ duration: isMoving ? 0 : 200 }}
    >
      <MoveItem2
        bind:value={item}
        {itemView}
