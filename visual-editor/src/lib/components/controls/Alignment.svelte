<script lang="ts">
  ...
  export let height:
    ...
    | undefined = undefined;
  export let orientation: 'horizontal' | 'vertical' | 'overlap' | undefined =
    undefined;
  ...
  $: isHorizontalDisabled =
    ...
    (!width ||
      ...
      (parentType === 'gallery' &&
        (!parentOrientation || parentOrientation === 'horizontal')));
  $: isVerticalDisabled =
    isSelfAlign &&
    (height?.type === 'match_parent' ||
      (parentType === 'container' &&
        (!parentOrientation || parentOrientation === 'vertical')) ||
  ...
  function selectAlignment(
    newHorizontal: string | undefined,
    newVertical: string | undefined,
  ): void {
    if (disabled) {
      return;
    }
...
>
  ...
  <div
    ...
    class:alignment__grid_disabled={disabled ||
      (isHorizontalDisabled && isVerticalDisabled)}
    tabindex={disabled || (isHorizontalDisabled && isVerticalDisabled)
      ? undefined
      : 0}
    ...
    {#each verticalList as verticalItem}
      {#each horizontalList as horizontalItem}
        ...
        <div
          ...
          class:alignment__val_selected={horizontal === horizontalItem &&
            vertical === verticalItem &&
            !(isHorizontalDisabled && isVerticalDisabled)}
          on:click|preventDefault={() =>
            selectAlignment(horizontalItem, verticalItem)}
          on:mouseenter={() => {
            if (!disabled) {
              isHovered = true;
  ...
  <div class="alignment__item alignment__text">
    ...
    {#if isContent && orientation}
      <div
        ...
        class:alignment__preview_halign_center={horizontalAlignHovered ===
          'center'}
        class:alignment__preview_halign_right={horizontalAlignHovered ===
          'right'}
        class:alignment__preview_valign_top={verticalAlignHovered === 'top'}
        class:alignment__preview_valign_center={verticalAlignHovered ===
          'center'}
        class:alignment__preview_valign_bottom={verticalAlignHovered ===
          'bottom'}
      >
...
<style>
  ...
  .alignment__grid:not(.alignment__grid_disabled)
