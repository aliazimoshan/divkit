<script lang="ts">
  ...
  $: items = fullComponentsList
    .concat(
      $userDefinedTemplates.map((type) => ({
        name: type,
        type: type,
        isTemplate: true,
      })),
...
<div
  ...
  class:tree-leaf_container-child={!isSimpleElement(
    leaf.parent?.props.json.type,
  )}
  ...
  <div class="tree-leaf__type-icon-outer">
    <div
      ...
      style:background-image="url({encodeBackground(
        state.componentIcon(leaf.props.json.type, leaf.props.json),
      )})"
    ></div>
  </div>
  {getText(leaf)}
...
{#if $dragTarget?.node === node && $dragTarget?.type === 'inside'}
  <div
    class="tree-leaf__drag-target tree-leaf__drag-target_inside"
    style="--level: {level + 1}"
  ></div>
{/if}
...
{#if $dragTarget?.node === node && $dragTarget?.type === 'after'}
  <div
    class="tree-leaf__drag-target tree-leaf__drag-target_after"
    style="--level: {level}"
  ></div>
{/if}
...
<style>
  ...
  .tree-leaf:not(.tree-leaf_selected).tree-leaf_errors:not(
      .tree-leaf_selected
    ):hover,
  ...
  .tree-leaf:not(.tree-leaf_selected).tree-leaf_warnings:not(
      .tree-leaf_selected
    ):hover,
