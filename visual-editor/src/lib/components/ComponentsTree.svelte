...
<script lang="ts">
  ...
  function onDragOver(event: DragEvent): void {
    ...
    const childs = Array.from(
      rootNode.querySelectorAll<HTMLElement>('.tree-leaf_container-child'),
    ).map((node) => {
    ...
    const containers = Array.from(
      rootNode.querySelectorAll<HTMLElement>('.tree-leaf_can-have-childs'),
    ).map((node) => {
      const bbox = node.getBoundingClientRect();
    ...
    childs.forEach(({ node, bbox }) => {
      ...
      const distBefore =
        pageY >= topBefore && pageY <= bottomBefore
          ? 0
          : Math.min(
      ...
      const distAfter =
        pageY >= topAfter && pageY <= bottomAfter
          ? 0
          : Math.min(Math.abs(pageY - topAfter), Math.abs(pageY - bottomAfter));
  ...
  function onDragLeave(event: DragEvent): void {
    if (
      ...
      rootNode.contains(event.relatedTarget)
    ) {
      return;
    }
    dragTarget.set(null);
  ...
  function onDrop(event: DragEvent): void {
    ...
    if (
      (best.type === 'before' || best.type === 'after') &&
      !leafTarget.parent
    ) {
      return;
    }
    ...
    if (best.type === 'inside') {
      ...
      insertIndex =
        leafTarget.parent.childs.indexOf(leafTarget) +
        (best.type === 'before' ? 0 : 1);
    }

    if (
      best.type !== 'inside' &&
      movedChild.parent === leafTarget.parent &&
