<script lang="ts">
  ...
  $: {
    ...
    if (baseType) {
      ...
      const templateBaseType = template
        ? getTemplateBaseType(baseType)
        : undefined;
      // eslint-disable-next-line no-nested-ternary
      const desc = template
        ? templateBaseType
          ? getSchemaByType(templateBaseType)
          : undefined
        ...
        props = allOf
          ...
          .map((it) => {
            ...
            return isTemplate(baseType)
              ? templateBaseType
                ? getSchemaNameByType(templateBaseType)
                : undefined
...
<div class="props">
  {#if showComplex}
    <PanelTitle
      title={$l10n(
        showComplex ? 'complexComponentProperties' : 'componentProperties',
      )}
    />
  {/if}
