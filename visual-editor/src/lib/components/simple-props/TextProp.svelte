<script lang="ts">
  ...
  const { getTranslationKey, getSelection, state, tanker2Dialog } =
  ...
  $: step =
    flags.subtype === 'integer' ? 1 : flags.subtype === 'number' ? 0.01 : null;
...
{#if tankerToggled}
  <div transition:slide|local>
    ...
    {:else if isTankerEditable}
      <div class="text-prop__tanker">
        <button class="text-prop__tanker-key" on:click={onLockedClick}>
