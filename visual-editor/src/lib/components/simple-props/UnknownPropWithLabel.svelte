<script lang="ts">
  ...
  const { cardLocales, state, expression2Dialog } =
  ...
  $: isExpression =
    !tankerToggled && value !== evalValue && typeof value !== 'object';
  $: showSources =
    !tankerToggled &&
    (('enableSources' in item && item.enableSources) || isExpression);
  ...
  $: hasLabel =
    ('name' in item && item.name) || ('rawName' in item && item.rawName);
...
<svelte:element this={isLabel ? 'label' : 'div'} class="unknown-prop">
  ...
  {#if hasLabel && isExpression}
    <ExpressionValue {evalValue} on:click={onSourceClick} />
  {:else}
    <UnknownProp
      {item}
      {value}
