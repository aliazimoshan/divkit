  ...
  function updateVal(value: Background): void {
    ...
    if (value.type === 'solid') {
      ...
      if (isPaletteColor(value.color)) {
        ...
        if (!paletteItem) {
          error = $l10nString('missingPaletteColor').replace(
            '%s',
            () => paletteId,
          );
        }
      ...
      if (Array.isArray(value.colors)) {
        preview = `linear-gradient(${90 - (value.angle || 0)}deg, ${value.colors
          .map((color) => {
...
<div
  ...
  title={error}
>
  <button class="background2-item__preview" on:click={onPreviewClick}>
    {#if value.type === 'solid'}
      <ColorPreview color={value.color} mix="background2-item__preview-inner" />
    {:else}
      <div class="background2-item__preview-bg"></div>
      <div
        class="background2-item__preview-value"
        style:background-image={preview}
      ></div>
  ...
  {:else}
    {#if value.type === 'solid'}
      ...
      />
    {:else}
      <button class="background2-item__text" on:click={onPreviewClick}>
...
<style>
  ...
  :global(
    .background2-item__preview-inner.background2-item__preview-inner.background2-item__preview-inner
  ) {
    position: absolute;
    top: 0;
    left: 0;
