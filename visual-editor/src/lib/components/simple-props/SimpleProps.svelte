<script lang="ts">
  ...
  import {
    ...
    type SiblingComponentProperty,
  } from '../../data/componentProps';
  import SimplePropsList from './SimplePropsList.svelte';
  import {
    SetPropertyCommand,
    type SetPropertyItem,
  } from '../../data/commands/setProperty';
  import { APP_CTX, type AppContext } from '../../ctx/appContext';
  import { getTemplateProps } from '../../data/userTemplates';

  const { rootConfigurable, state, rendererApi } =
  ...
  $: parentProcessedJson =
    $selectedLeaf?.parent?.props.processedJson ||
    $selectedLeaf?.parent?.props.json ||
    null;
  $: evalJson =
    (processedJson && rendererApi().evalJson(processedJson)) || null;
  $: parentEvalJson =
    (parentProcessedJson && rendererApi().evalJson(parentProcessedJson)) ||
    null;
  $: type = json?.type;
  $: baseType = state.getBaseType(type);
  $: list =
    (!rootConfigurable && $selectedLeaf === $tree && ROOT_PROPS) ||
    (namedTemplates[type]?.props && [
      ...BASE_COMPONENT_PROPS,
      ...namedTemplates[type]?.props,
    ]) ||
  ...
  async function onChange(
    ...
    const pushChange = ({
      ...
      if (property === 'width.type' || property === 'height.type') {
        ...
        const sizeProp =
          property === 'alignment_horizontal' ? 'width' : 'height';
