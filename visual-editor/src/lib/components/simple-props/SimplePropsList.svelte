...
<div class="simple-props-group">
  <ul class="simple-props-group__list">
    {#each propsList as item}
      ...
      {:else}
        {@const value =
          item.prop && getProp(processedJson, item.prop, item.default)}
        {@const evalValue =
          item.prop && getProp(evalJson, item.prop, item.default)}
        {@const enabled =
          item.enabled === false
            ? false
            : !item.enabled ||
          ...
          {#if item.siblings}
            {#each item.siblings as sibling}
              {#if !('show' in sibling && sibling.show) || evalCondition(processedJson, parentProcessedJson, sibling.show)}
                {@const value =
                  'sizeValue' in sibling && sibling.sizeValue
                    ? formatSize(selectedElemProps?.[sibling.sizeValue])
                    : 'prop' in sibling &&
                      sibling.prop &&
                      getProp(processedJson, sibling.prop, sibling.default)}
                {@const evalValue =
                  'sizeValue' in sibling && sibling.sizeValue
                    ? formatSize(selectedElemProps?.[sibling.sizeValue])
                    : 'prop' in sibling &&
                      sibling.prop &&
                      getProp(evalJson, sibling.prop, sibling.default)}
