  ...
  function warnLabel(
    ...
    if (
      ...
      if (desc.platforms.length) {
        list.push(
          $l10n('props.supports') + desc.platforms.map(capitalize).join(', '),
        );
      } else {
...
{#each list as item}
  ...
  <li
    ...
    class:props-group__deprecated={deprecated &&
      item.type !== 'group' &&
      item.type !== 'list'}
  >
    {#if item.type === 'group'}
      <div class="props-group__span-cell">
        <Spoiler2
          ...
          mix="props-group__group-summary {filled
            ? 'props-group__filled'
            : ''} {deprecated ? 'props-group__deprecated' : ''}"
          open={false}
        >
          <div slot="title">
            {#if item.desc.platforms || item.desc.deprecated}
              ...
              {#if warn}
                <span class="props-group__warning" title={warn}></span>
              {/if}
            {/if}
            {item.name}
          ...
          <ul class="props-group__content">
            {#if item.options}
              <li
                ...
                title="{item.name}: {item.description?.ru}"
              >
                <label for={id} class="props-group__name"> type </label>
            ...
            {#if item.additionalProperties}
              {#each Object.keys(value || {}) as key}
                ...
                <li class="props-group__item" title={key}>
                  <div
                    {...{ autocomplete: 'off' }}
                    ...
                    spellcheck="false"
                    on:input={(event) =>
                      onCustomNameChange(event, item.prop, key)}
                  >
                    {key}
                  </div>
                  <div class="props-group__value">
              ...
              {#if !$readOnly}
                <button
                  ...
                  on:click={() =>
                    dispatch('add', { prop: item.prop, subtype: 'custom' })}
                >
                  {$l10n('props.add')}
                </button>
              {/if}
        ...
        >
          ...
          <ul class="props-group__content">
            {#if item.list}
              {#each item.list as item2, index}
                <li
                  class="props-group__item"
                  title="{item2.name}: {item2.description?.ru}"
                >
                  <div class="props-group__span-cell">
                    <Spoiler2
                      theme="straight-small"
          ...
          {#if !$readOnly}
            <button
              ...
              on:click={() =>
                dispatch('add', { prop: item.prop, subtype: item.subtype })}
            >
              {$l10n('props.add')}
            </button>
          {/if}
    ...
    {:else}
      ...
      <div class="props-group__value">
        ...
        {:else if item.type === 'checkbox'}
          <CheckboxProp {value} {id} prop={item.prop} on:change />
        {/if}
      </div>
    {/if}
  </li>
