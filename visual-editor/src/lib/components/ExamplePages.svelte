  ...
  {:else}
    <div>
      ...
      {#each $systemsList as system (system.id)}
        ...
        >
          <h4>({system.id}) - {capitalize(system.systemName)}</h4>
              ...
              {#each $systemScreens[system.id] as systemScreen (systemScreen.id)}
                ...
                >
                  <div class="screen-item">
                    <!-- Delete button for screen -->
                    <!-- <button
                      on:click={(event) => {
                        event.stopPropagation(); // Prevent triggering system select
                        isYesNoModalOpen.set(true);
                        deletingScreen.set(systemScreen);
                      }}
                    >
...
<style>
  ...
  .screen-item h3 {
    ...
    font-weight: bold;
    color: #333; /* Darker color for screen name */
  }

  .screen-item .screen-subtext {
    margin-top: 2px;
    font-size: 16px;
    width: 100%;
    text-align: left;
    color: gray; /* Lighter color for the subtext */
  }

  .screen-item button {
    background: none; /* Remove default button background */
    border: none; /* Remove border */
    color: #ff4d4d; /* Optional: color for delete icon */
