<script lang="ts">
  ...
  } from '@divkitframework/divkit/typings/variables';
  import {
    getLeafDepth,
    getDepth,
    type HighlightMode,
  ...
  import {
    ...
    moveComponentRight,
    bigMoveComponentUp,
    bigMoveComponentDown,
    bigMoveComponentRight,
    bigMoveComponentLeft,
    resizeComponentUp,
    resizeComponentDown,
    resizeComponentLeft,
    resizeComponentRight,
    bigResizeComponentUp,
    bigResizeComponentDown,
    bigResizeComponentRight,
    bigResizeComponentLeft,
    copy as copyShortcut,
    paste as pasteShortcut,
    cancel,
  } from '../utils/keybinder/shortcuts';
  ...
  const updateHighlight = (
    ...
    if (bestDragTarget || showInplaceEditor) {
      ...
      elems = [...new Set([...(highlightElems || []), selectedElem])].filter(
        Truthy,
      );
      if (
        highlightElems &&
    ...
    highlights = elems
      ...
      .map((elem) => {
        ...
        if (elem === clone) {
          ...
          const parentRotation = elem.parentElement
            ? calcRotationRad(elem.parentElement)
            : 0;
        ...
        if (elem === selectedElem && processedJson) {
          ...
          if (json) {
            if (json.type === '_template_lottie') {
              ...
              processedJson.type ===
              'image' /*  && processedJson.image_url === EMPTY_IMAGE */
            ) {
              emptyFileType = 'image';
            } else if (
              processedJson.type ===
              'gif' /*  && processedJson.gif_url === EMPTY_IMAGE */
            ) {
              emptyFileType = 'gif';
            }
          }
        ...
        return {
          ...
          rotation:
            clone === elem
              ? componentCloneRotation
              : (calcRotationRad(elem) / Math.PI) * 180,
          emptyFileType,
          insets,
          ...
          grid:
            (gridProps &&
              (gridResize || {
                columns: sizesToFall(
                  gridProps.columns.slice(0, gridProps.columns.length - 1),
                ),
                rows: sizesToFall(
                  gridProps.rows.slice(0, gridProps.rows.length - 1),
                ),
              })) ||
            undefined,
        };
      ...
      for (const side of ['top', 'right', 'bottom', 'left'] as const) {
        ...
        for (const oppositeSide of [INVERSE[side], side]) {
          if (fromBbox[side] * multiply < toBbox[oppositeSide] * multiply) {
            let sub;
            const middle =
              (fromBbox[MIDDLE[orientation][0]] +
                fromBbox[MIDDLE[orientation][1]]) /
              2;
            ...
            if (middle < toBbox[MIDDLE[orientation][0]]) {
              sub = {
                ...
                top:
                  subOrientation === 'vertical' ? middle : toBbox[oppositeSide],
                left:
                  subOrientation === 'horizontal'
                    ? middle
                    : toBbox[oppositeSide],
              };
              ...
              sub = {
                ...
                top:
                  subOrientation === 'vertical'
                    ? toBbox[MIDDLE[orientation][1]]
                    : toBbox[oppositeSide],
                left:
                  subOrientation === 'horizontal'
                    ? toBbox[MIDDLE[orientation][1]]
  ...
  $: {
    ...
    const elems = [
      ...new Set([...($highlightElem || []), $selectedElem].filter(Truthy)),
    ];
    if (elems.length) {
  ...
  function onComponent({
    ...
    if ((type === 'mount' || type === 'update') && origJson.__leafId) {
      ...
      if (leaf) {
        ...
        const isSelected =
          $selectedLeaf && $selectedLeaf.id === origJson.__leafId;
        ...
        if (node) {
          ...
          node.classList.toggle(
            'renderer__divkit-gallery',
            json.type === 'gallery',
          );
          node.classList.toggle(
            'renderer__divkit-pager',
            json.type === 'pager',
          );
  ...
  function getBestDropTarget(x: number, y: number) {
    ...
    for (const elem of elems) {
      ...
      if (
        ...
        !(
          processedJson.type === 'container' ||
          processedJson.type === 'gallery' ||
          processedJson.type === 'grid'
        ) ||
      ...
      const orientation =
        processedJson.orientation ||
        (processedJson.type === 'container' ? 'vertical' : 'horizontal');
        ...
        Array.from(children).forEach((child, index) => {
          ...
          targets.push({
            ...
            top:
              orientation === 'horizontal'
                ? bbox.top + paddingTop
                : childBbox.bottom + marginBottom,
            left:
              orientation === 'horizontal'
                ? childBbox.right + marginRight
                : bbox.left + paddingLeft,
            width:
              orientation === 'horizontal'
                ? 0
                : bbox.width - paddingLeft - paddingRight,
        ...
        const distX =
          x < target.left
            ...
            : x < target.left + target.width
              ? 0
              : Math.abs(x - (target.left + target.width));
        // eslint-disable-next-line no-nested-ternary
        const distY =
          y < target.top
            ? target.top - y
            : y < target.top + target.height
              ? 0
              : Math.abs(y - (target.top + target.height));
        const dist = distX * distX + distY * distY;

        if (
          !best ||
          depth > best.depth ||
          (depth === best.depth && best.dist > dist)
        ) {
  ...
  function onDragOver(event: DragEvent): void {
    ...
    bestDragTarget =
      (!event.ctrlKey && getBestDropTarget(event.clientX, event.clientY)) ||
      null;
  ...
  function rerender(
    ...
    if (instance) {
      ...
      if (paletteVariable) {
        const variableDesc = divjson.card.variables?.find(
          (it: JsonVariable) => it.name === 'local_palette',
        );
      ...
      if (actionLogUrlVariable) {
        const logVariable = createVariable(
          actionLogUrlVariable,
          'string',
          'https://',
        );
      ...
      if (createdTankerVars.size) {
        ...
        if (card.variables) {
          card.variables = card.variables.filter(
            (it) => !createdTankerVars.has(it.name),
          );
        }
      }
  ...
  $: if (
    $divjsonStore.object.card?.states?.[0]?.div &&
    rootPreview &&
    $locale !== undefined
  ) {
  ...
  function onPointerDown(event: PointerEvent): void {
    ...
    if (
      ...
      (event.target.matches(
        '.renderer__divkit-gallery > div:not(:first-child) *',
      ) ||
        event.target.matches(
          '.renderer__divkit-pager > div:not(:first-child) *',
        ) ||
    ...
    const pointermove = (event: PointerEvent) => {
      const dist =
        (event.clientX - startEventX) * (event.clientX - startEventX) +
      ...
      if (bestDragTarget) {
        ...
        targetX =
          (snappedTargetX &&
            snapXOffset !== undefined &&
            snappedTargetX.val - snapXOffset) ||
          targetX;
        targetY =
          (snappedTargetY &&
            snapYOffset !== undefined &&
            snappedTargetY.val - snapYOffset) ||
          targetY;
    ...
    const pointerup = () => {
      ...
      if (dragTarget) {
        ...
        const newParentJson =
          leafTarget && findLeaf($tree, leafTarget.id)?.props.processedJson;
        if (
        ...
        if (leafTarget && !treeLeafContains(leaf, leafTarget)) {
          ...
          state.pushCommand(
            new MoveLeafCommand(state, {
              ...
              newParentId: leafTarget.id,
              insertIndex:
                insertIndex !== null ? insertIndex : leafTarget.childs.length,
              changes,
            }),
          );
        }
        ...
        const isMatchParentWidth =
          processedJson.width?.type === 'match_parent' || !processedJson.width;
        const isMatchParentHeight =
          processedJson.height?.type === 'match_parent';
        ...
        if (moveX) {
          ...
          if (hAlign === 'right' || isMatchParentWidth) {
            margins.right = Math.floor(
              containerBbox.width - targetX - startBbox.width,
            );
          }
        } else {
        ...
        if (moveY) {
          ...
          if (vAlign === 'bottom' || isMatchParentHeight) {
            margins.bottom = Math.floor(
              containerBbox.height - targetY - startBbox.height,
            );
          }
        } else {
  ...
  function onResize(
    ...
    const gridIsMatchParentWidth =
      isGridChild &&
      (!parentLeaf.props.processedJson.width ||
        parentLeaf.props.processedJson.width.type === 'match_parent');
    const gridIsMatchParentHeight =
      isGridChild &&
      parentLeaf.props.processedJson.height.type === 'match_parent';
    let gridProps: GridProps | undefined;
    ...
    const gridMaxColumnSpan =
      gridProps && gridPosition
        ? gridProps.columnsCount - gridPosition.columnStart
        : 0;
    const gridMaxRowSpan = gridProps
      ? /* gridProps.rowsCount - gridPosition.rowStart */ Infinity
      : 0;
    const gridMaxItemWidth =
      gridProps && gridPosition
        ? gridProps.columns[gridPosition.columnStart] -
          parseFloat(computedStyle.marginLeft) -
          parseFloat(computedStyle.marginRight)
        : 0;
    const gridMaxItemHeight =
      gridProps && gridPosition
    ...
    const topLeftCorner = {
      x:
        center.x +
        (-offsetWidth / 2) * rotationCos -
        (-offsetHeight / 2) * rotationSin,
      y:
        center.y +
        (-offsetWidth / 2) * rotationSin +
        (-offsetHeight / 2) * rotationCos,
    };
    ...
    const pointermove = (event: PointerEvent) => {
        ...
        if (!gridProps || !gridPosition || calcedWidth <= gridMaxItemWidth) {
          ...
          for (columnSpan = 0; columnSpan < gridMaxColumnSpan; ++columnSpan) {
            const currentColumnWidth =
              gridProps.columns[gridPosition.columnStart + columnSpan];
      ...
      if (isYResize) {
        if (!gridProps || !gridPosition || calcedHeight <= gridMaxItemHeight) {
          ...
          for (rowSpan = 0; rowSpan < gridMaxRowSpan; ++rowSpan) {
            const currentColumnHeight = getRowHeight(
              gridPosition.rowStart + rowSpan,
            );
          ...
          targetHeight = 0;
          for (
            let i = gridPosition.rowStart;
            i < gridPosition.rowStart + rowSpan;
            ++i
          ) {
      ...
      if (componentCloneRotation && !isGridChild) {
        ...
        const topLeftCorner2 = {
          x:
            center.x +
            (-targetWidth / 2) * rotationCos -
            (-targetHeight / 2) * rotationSin,
          y:
            center.y +
            (-targetWidth / 2) * rotationSin +
            (-targetHeight / 2) * rotationCos,
        };
  ...
  function onGridResize(
    ...
    const pointerup = () => {
      ...
      const prevSizes =
        gridProps[type === 'column' ? 'columns' : 'rows'].slice();
      ...
      leaf.childs.forEach((child) => {
        ...
        if (
          ...
          if (child.props.processedJson[sizeProp]?.type === 'fixed') {
            ...
            childSize -=
              Number(margins?.left || 0) + Number(margins?.right || 0);
          } else {
            childSize -=
              Number(margins?.top || 0) + Number(margins?.bottom || 0);
          }
          if (
            child.props.processedJson[sizeProp]?.type === 'fixed' &&
            Math.abs(childSize - child.props.processedJson[sizeProp].value) < 1
          ) {
            return;
          }
          changes.push({
  ...
  function moveComponentOrder(
    ...
    const parentOrientation =
      parent.props.processedJson.orientation ||
  ...
  function resizeComponent(
    ...
    const maxSize =
      parentBbox[prop === 'width' ? 'right' : 'bottom'] -
    ...
    const newSize = Math.max(
      ...
      Math.min(
        maxSize,
        Math.round(currentSize + resize[prop === 'width' ? 'x' : 'y']),
      ),
    );
  ...
  function onKeydown(event: KeyboardEvent): void {
    ...
    const moveUp =
      moveComponentUp.isPressed(event) || bigMoveComponentUp.isPressed(event);
    const moveDown =
      moveComponentDown.isPressed(event) ||
      bigMoveComponentDown.isPressed(event);
    const moveLeft =
      moveComponentLeft.isPressed(event) ||
      bigMoveComponentLeft.isPressed(event);
    const moveRight =
      moveComponentRight.isPressed(event) ||
      bigMoveComponentRight.isPressed(event);

    const isBigMove =
      bigMoveComponentUp.isPressed(event) ||
      bigMoveComponentDown.isPressed(event) ||
      bigMoveComponentLeft.isPressed(event) ||
      bigMoveComponentRight.isPressed(event);

    const resizeUp =
      resizeComponentUp.isPressed(event) ||
      bigResizeComponentUp.isPressed(event);
    const resizeDown =
      resizeComponentDown.isPressed(event) ||
      bigResizeComponentDown.isPressed(event);
    const resizeLeft =
      resizeComponentLeft.isPressed(event) ||
      bigResizeComponentLeft.isPressed(event);
    const resizeRight =
      resizeComponentRight.isPressed(event) ||
      bigResizeComponentRight.isPressed(event);
      ...
      if (
        ...
        for (const key in applyRangeToRoot) {
          if (
            key !== 'start' &&
            key !== 'end' &&
            json[key] !== applyRangeToRoot[key as keyof TextRange]
          ) {
            changes.push({
              property: key,
        ...
        const filtered = ranges.filter(
          (range) => !isRangeStyleEqual(range, applyRangeToRoot),
        );
        if (filtered.length) {
          filtered.forEach((range) => {
            for (const key in range) {
              if (
                key !== 'start' &&
                key !== 'end' &&
                range[key as keyof TextRange] ===
                  applyRangeToRoot[key as keyof TextRange]
              ) {
      ...
      state.pushCommand(
        new SetPropertyCommand($tree, [
          ...
          },
          {
            ...
            value:
              newState.textAlign !== 'left' ? newState.textAlign : undefined,
          },
          ...changes.map((change) => ({
            leafId: value.leaf.id,
  ...
  <div class="renderer__overlay-elemens renderer__overlay-root">
    ...
    >
      ...
      <div class="renderer__snap-container">
        {#if snapLineX}
          <div
            class="renderer__snap renderer__snap_vertical"
            style:left="{snapLineX}px"
          ></div>
        {/if}
        {#if snapLineY}
          <div
            class="renderer__snap renderer__snap_horizontal"
            style:top="{snapLineY}px"
          ></div>
        {/if}
      </div>
              ...
              >
                {#if highlight.insets}
                  <svg
                    ...
                    width={highlight.widthNum +
                      highlight.margins.left +
                      highlight.margins.right}
                    height={highlight.heightNum +
                      highlight.margins.top +
                      highlight.margins.bottom}
                    ...
                    {#each highlight.grid.columns as column, index}
                      <div
                        ...
                        on:pointerdown|stopPropagation|preventDefault={(
                          event,
                        ) => onGridResize(event, 'column', index)}
                      ></div>
                    {/each}
                ...
                {#if highlight.permanent && !highlight.isRoot && !highlight.insets && !$readOnly}
                  <div
                    class="renderer__highlight-border-bottom-right"
                    on:pointerdown|stopPropagation|preventDefault={(event) =>
                      onResize(event, 'down-right')}
                  ></div>
                  <div
                    class="renderer__highlight-border-right"
                    on:pointerdown|stopPropagation|preventDefault={(event) =>
                      onResize(event, 'right')}
                  ></div>
                  <div
                    class="renderer__highlight-border-bottom"
                    on:pointerdown|stopPropagation|preventDefault={(event) =>
                      onResize(event, 'down')}
                  ></div>
                {/if}
                {#if (highlight.permanent || highlight.isRotate) && !highlight.isRoot && !highlight.insets && !$readOnly}
                  <div
                    class="renderer__highlight-border-rotate"
                ...
                {#if highlight.isRotate}
                  ...
                  <div
                    ...
                    style:--offset-x="{54 *
                      Math.cos(degToRad(270 - componentCloneRotation / 2))}px"
                    style:--offset-y="{54 *
                      Math.sin(degToRad(270 - componentCloneRotation / 2))}px"
                    style:--rotation="{-componentCloneRotation}deg"
                  >
                  ...
                  >
                    <path
                      d="M 32 {32 - 24} A 24 24 0 {componentCloneRotation >= 180
                        ? 1
                        : 0} 0 {32 +
                        24 *
                          Math.cos(
                            degToRad(-componentCloneRotation - 90),
                          )} {32 +
                        24 * Math.sin(degToRad(-componentCloneRotation - 90))}"
        ...
        {#each highlightDistance as item}
          <div class="renderer__highlight-inner">
            <div
              ...
              style:width={item.orientation === 'horizontal'
                ? `${item.length}px`
                : ''}
              style:height={item.orientation === 'horizontal'
                ? ''
                : `${item.length}px`}
              style:margin-top="{-scrollY}px"
              style:margin-left="{-scrollX}px"
            ...
            {#if item.sub}
              <div
                class="renderer__highlight-distance renderer__highlight-distance_sub renderer__highlight-distance_orientation_{item
                  .sub.orientation}"
                style:left="{item.sub.left}px"
                style:top="{item.sub.top}px"
                style:width={item.sub.orientation === 'horizontal'
                  ? `${item.sub.length}px`
                  : ''}
                style:height={item.sub.orientation === 'horizontal'
                  ? ''
                  : `${item.sub.length}px`}
                style:margin-top="{-scrollY}px"
                style:margin-left="{-scrollX}px"
...
<style>
  ...
  :global(
    .renderer__divkit-gallery > div > div > div > .renderer__divkit-node
  ) {
    pointer-events: none;
  }
