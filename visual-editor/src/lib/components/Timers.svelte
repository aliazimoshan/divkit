<script lang="ts">
  ...
  function onValueVariableChange(
    index: number,
    event: CustomEvent<{
      value: string | number;
    }>,
  ...
  function onTickActions(
    ...
    const timer = { ...$timers[index] };
    timer.tick_actions = event.detail.values.find(
      (it) => it.prop === 'actions',
    )?.value;
    const newList = $timers.slice();
    newList[index] = timer;
  ...
  function onEndActions(
    ...
    const timer = { ...$timers[index] };
    timer.end_actions = event.detail.values.find(
      (it) => it.prop === 'actions',
    )?.value;
    const newList = $timers.slice();
    newList[index] = timer;
...
<div class="timers">
  ...
  {#if !$readOnly}
    <AddButton cls="timers__add" disabled={$readOnly} on:click={add}>
