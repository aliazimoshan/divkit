<script lang="ts">
  ...
  function onWindowClick(event: MouseEvent): void {
    ...
    if (
      !path.includes(controllerElem) &&
      !(currentProps?.owner && path.includes(currentProps.owner))
    ) {
      hide();
    }
  }
  ...
  function calcStyle(
    ...
    const targetLeft =
      direction === 'bottom' ? ownerLeft : ownerLeft + ownerWidth;
    if (targetLeft + popupWidth < windowWidth) {
      left = targetLeft;
    } else if (ownerRight > popupWidth) {
      left = ownerRight - popupWidth;
    } else if (
      ownerLeft - (windowWidth - popupWidth) <
      popupWidth - ownerRight
    ) {
