<script lang="ts">
  ...
  import {
    undo as undoShortcut,
    redo as redoShortcut,
  ...
  const { cardLocales, state, setShowErrors, showErrors } =
  ...
  $: viewportList = [...VIEWPORT_LIST, 'custom'].map((it) => ({
    value: it,
  ...
  function onViewportHeightChange(
    event: CustomEvent<{
      value: string;
    }>,
  ): void {
...
>
  <div class="canvas__topbar" bind:this={topbar}>
    ...
    <CanvasButton
      title={$l10n('undo') +
        ($currentUndoStore ? ' — ' + $l10n($currentUndoStore.toLangKey()) : '')}
    ...
    <CanvasButton
      title={$l10n('redo') +
        ($currentRedoStore ? ' — ' + $l10n($currentRedoStore.toLangKey()) : '')}
  ...
  <div class="canvas__renderer-wrapper" bind:this={wrapper}>
    <Renderer bind:viewport theme={$previewThemeStore} />
  </div>
</div>
