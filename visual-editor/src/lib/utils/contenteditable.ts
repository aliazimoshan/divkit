...
export function calcSelectionOffset(
  ...
  try {
    ...
    if (node instanceof HTMLElement) {
      let sum = 0;
      for (
        let i = 0, len = Math.min(res, node.childNodes.length);
        i < len;
        ++i
      ) {
...
export function getInnerText(target: Node, includeImages: boolean): string {
  ...
  if (
    childs.length === 1 &&
    childs[0] instanceof HTMLElement &&
    childs[0].tagName === 'BR'
  ) {
    return '\n';
  }
  ...
  for (let i = 0, len = childs.length; i < len; ++i) {
    ...
    if (
      ...
      child.tagName === 'DIV' &&
      i > 0 &&
      (prevChild.nodeType === STRING_NODE_TYPE ||
        (prevChild instanceof HTMLElement &&
          (prevChild.tagName === 'A' || prevChild.tagName === 'SPAN')))
    ) {
      res += '\n';
    }
