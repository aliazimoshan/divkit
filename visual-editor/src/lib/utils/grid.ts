...
export function getGridProps(grid: HTMLElement): GridProps {
  ...
  children.forEach((child, index) => {
    ...
    const columnSpan = computed.gridColumnEnd.startsWith('span ')
      ? Number(computed.gridColumnEnd.split(' ')[1])
      : 1;
    const rowSpan = computed.gridRowEnd.startsWith('span ')
      ? Number(computed.gridRowEnd.split(' ')[1])
      : 1;
    ...
    let height =
      childBbox.height + topMargin + parseFloat(computed.marginBottom);
  ...
  const columnsStart = columns.reduce(
    (acc, item) => {
