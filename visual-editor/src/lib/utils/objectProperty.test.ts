...
describe('objectProperty', () => {
  test('getObjectProperty', () => {
    ...
    expect(getObjectProperty({ name: 'test' }, 'name.title')).toEqual(
      undefined,
    );
    expect(getObjectProperty({ name: 'test' }, 'texts.title')).toEqual(
      undefined,
    );

    expect(getObjectProperty({ name: [0, 1, 2] }, 'texts[1]')).toEqual(
      undefined,
    );
    expect(getObjectProperty({ name: [0, 1, 2] }, 'name[1]')).toEqual(1);

    expect(getObjectProperty({ name: [0, 1, 2] }, 'name[4]')).toEqual(
      undefined,
    );

    expect(
      getObjectProperty({ name: [{ title: 'hello' }] }, 'name[0].title'),
    ).toEqual('hello');
  });
