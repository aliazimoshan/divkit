...
function nameToCode(name: string): string {
  return name
    .split('')
    .map((char) => `u${String(char.charCodeAt(0)).padStart(4, '0')}`)
    .join('');
}
...
function codeToName(code: string): string {
  return code
    .split('u')
    .map((part) => (part ? String.fromCharCode(Number(part)) : ''))
    .join('');
}
...
export function convertDictToPalette(json: any) {
  ...
  const dictPalette = json.card.variables?.find(
    (it: JsonVariable) => it.name === 'local_palette',
  )?.value;

  if (dictPalette) {
    json.card.variables = json.card.variables.filter(
      (it: JsonVariable) => it.name !== 'local_palette',
    );
