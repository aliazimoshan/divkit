...
export const schema = Object.keys(jsons).reduce(
  (acc, item) => {
    acc[item.replace('../../../schema/', '').replace(/\.json$/, '')] = jsons[
      item
    ] as SchemaModule;
    return acc;
  },
...
export function filterProp<T extends Record<string, unknown>>(
  obj: T,
  prop: string,
): Partial<T> {
  ...
  return Object.keys(props)
    .filter((it) => {
      const res =
        !(
          it === 'type' &&
          props[it].type === 'string' &&
          Array.isArray(props[it].enum) &&
          props[it].enum.length === 1
        ) &&
...
export function getPropsElement(
  name: string,
  prop: Schema,
  componentJson: any,
  subpath = '',
  ...
  if (prop.type === 'array' && prop.items) {
    ...
    if (list) {
      ...
      list = Array.isArray(list)
        ? list.map((it2, index) => {
            if (resolvedAnyOf) {
              const selected = resolvedAnyOf.find(
                (it3) => it3?.properties?.type?.enum?.[0] === it2.type,
              );
  ...
  if (prop.anyOf) {
    ...
    if (componentJson?.type) {
      const selected = resolvedAnyOf.find(
        (it2) => it2?.properties?.type?.enum?.[0] === componentJson.type,
      );
