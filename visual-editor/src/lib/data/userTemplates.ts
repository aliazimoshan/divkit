      ...
      if (typeof obj === 'object' && obj) {
        for (const key in obj) {
          ...
          if (schemaObj?.properties && templatedKey in schemaObj.properties) {
            ...
            subSchema = schemaObj.anyOf.find(
              (it) => it.properties?.type?.enum?.[0] === obj.type,
          ...
          if (Array.isArray(val)) {
            const itemSchema =
              ...
              subSchema.items &&
              resolveObjectRelative(schemaObj, subSchema.items, 0);
            val.forEach((subitem) => {
  ...
  const proc = (
    ...
    if (typeof obj === 'object' && obj) {
      for (const key in obj) {
        ...
        const val = isTemplateKey
          ? templateProps.get(obj[key as keyof typeof obj])
