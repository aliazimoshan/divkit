...
export class State {
  ...
  currentUndoStore = derived(
    [this.#commands, this.#historyIndex],
    ([history, historyIndex]) => {
      return history[historyIndex];
    },
  );
  currentRedoStore = derived(
    [this.#commands, this.#historyIndex],
    ([history, historyIndex]) => {
  ...
  divjsonStore = derived(this.tree, (root) => {
    const { shortObject, mapJsonToLeaf } = this.treeToShortObject(root);
    ...
    return {
      object: fullObject,
      fullString: stringifyObjectAndStoreRanges({
        object: fullObject,
        mapJsonToLeaf,
      }),
    };
  ...
  divToLeaf(div: any, parent?: TreeLeaf, info?: unknown): TreeLeaf {
    ...
    if (type in namedTemplates) {
      ...
      ({ childs, infos, fromDataField } = parseItems(this, div));
    }
    ...
    node.childs = childs.map((it, index) =>
      this.divToLeaf(it, node, infos[index]),
    );
  ...
  setDivJson(json: any): void {
    ...
    if (Array.isArray(json.card?.variables)) {
      this.customVariables.set(
        this.extractVariables(json.card.variables, get(this.sources)),
      );

      json.card.variables.forEach((variable: JsonVariable) => {
        if (
          variable.name === 'local_palette' &&
          variable.value &&
  ...
  toFullDivjson(
    ...
    const card: Record<string, unknown> = (json.card ||
  ...
  genNewLeaf(type: string, isCentered = false): TreeLeaf {
    ...
    if (
      !isSimpleElement(type) &&
      !(type in namedTemplates) &&
      !this.userTemplates.has(type) &&
      type !== 'custom'
    ) {
  ...
  duplicateLeaf(
    leaf: TreeLeaf,
    parent: TreeLeaf | undefined = undefined,
  ): TreeLeaf {
    const copy: TreeLeaf = {
      id: this.genId(),
      props: { ...leaf.props },
