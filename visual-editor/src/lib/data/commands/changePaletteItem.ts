...
export class ChangePaletteItemCommand extends BaseCommand {
  ...
  undo(state: State): void {
    ...
    state.palette.set(
      setItemColor(list, this.id, this.theme, this.beforeColor),
    );
  }
  ...
  redo(state: State): void {
    ...
    if (item) {
      ...
      state.palette.set(
        setItemColor(list, this.id, this.theme, this.afterColor),
      );
    }
  }
  ...
  canMerge(other: BaseCommand): boolean {
    return (
      super.canMerge(other) &&
      other instanceof ChangePaletteItemCommand &&
      this.id === other.id &&
