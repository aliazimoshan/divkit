  ...
  update(highlights, tr: Transaction) {
    ...
    for (const effect of tr.effects) {
      if (effect.is(addHighlight)) {
        const startLine = tr.state.doc.lineAt(
          Math.min(len, effect.value.from),
        ).number;
        const endLine = tr.state.doc.lineAt(
          Math.min(len, effect.value.to),
        ).number;
        const list = [];
        for (let line = startLine; line <= endLine; ++line) {
          list.push(
            (effect.value.type === 'highlight'
              ? highlightMark
              : selectMark
  ...
  return {
    ...
    decorateRanges(typedRanges: TypedRange[]): void {
      ...
      if (typedRanges.length) {
        effects = typedRanges.map((typedRange) =>
          addHighlight.of({
