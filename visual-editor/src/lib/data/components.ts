...
const MAP = (schema.div.anyOf || [])
  .filter((it) => it.$ref !== 'div-switch.json')
  .reduce(
    (acc, item) => {
      if (item.$ref) {
        const type = getTypeBySchemaName(
          item.$ref.replace(/\.json$/, ''),
        ) as string;
...
export const smallComponentsList: ComponentListItem[] =
  DEFAULT_ALLOWED_COMPONENTS.map((type) => {
    return {
...
export const fullComponentsList: ComponentListItem[] = Object.keys(MAP).map(
  (type) => {
    return {
      nameKey: `components.${type}`,
      type,
